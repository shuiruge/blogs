...
Stimulating a kind of neurons may activate many neural pathways that are successively connected to the neurons. To selectively activate a neural pathway among the rest, it is essential to facilitate the synapses along the neural pathway, so that the it has much higher probability of activation than the others. The facilitation is mediated by some chemical mediators produced in the synapses. These chemical mediators will persist until being metabolized. In this way, the neural pathway is modulated. [^modulation]
...
_Anticipation_ is an intention to response to a kind of stimuli by a specific behavior. [^anticipation] It selectively activates the neural pathway, if exists, from the stimuli to the behavior. So, anticipation modulates neural pathway so that chemical mediators that facilitate synapse are produced in the synapses along the neural pathway.
...
Chemical mediators will be metabolized soon. But if an anticipation can be sustained, so is the modulation, the chemical mediators in the synapses along the neural pathway will be continually produced. Then, metabolism cannot stop the chemical mediators from accumulating. It does not take long for the concentrations to become so high as to trigger an intense synaptic plasticity, leading to long-lasting anatomical changes in the synapses. [^synaptic-plasticity] So, we can plant what we want to cultivate, or uproot what we want to eliminate, by simply sustaining a proper anticipation. [^hypnosis]
