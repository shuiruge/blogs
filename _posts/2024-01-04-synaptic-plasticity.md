...
_Anticipation_ is an intention to response to a kind of stimuli by a specific behavior. [^anticipation] It selectively activates the neural pathway, if exists, from the stimuli to the behavior. So, anticipation modulates the neural pathway so that the chemical mediators that facilitate the synapses along the neural pathway are activated and accumulated.
