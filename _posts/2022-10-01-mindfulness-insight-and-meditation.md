...
[^modularity]: A module may not be identified by a location in human brain. In fact, a _module_ is an agent that accomplishes one step of the perceptive process. So, a module can be a combination of many sub-modules, each accomplishing a more fundamental step, cooperating with each other through the consciousness in due order, so that the total effect is the step accomplished by the combined. And this module, combined with some others, can form a super-module. This is an effective theory, remind us of the Kadanoff's renormalization group in physics. And, it's in this perspective that any step of the perceptive process has a corresponding module that accomplishes it.
...
[^problem]: The problem is whether or not there are multiple awareness modules. For example, whether or not, for each sense door, there's one awareness module that only accomplishes the awareness of this sense door. This is critical.
...
[^awareness-remains-highly-active]: When awareness remains highly active, the perceptive process, in a simplified and indicative form, will be like
    ...
    where the terms "aware of A" and "aware of B" are packs of information projected from awareness. During a time quantum, once awareness arises, other objects will be excluded. So, all that are perceived are now realized.
